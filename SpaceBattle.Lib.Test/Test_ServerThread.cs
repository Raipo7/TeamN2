...
public class Test_ServerThread
{
    ...
    public object IoCdependency() {
        ...
        IoC.Resolve<ICommand>("IoC.Register", "Thread.GetIdByThreas", (object[] args) => {
        ...
        IoC.Resolve<ICommand>("IoC.Register", "Thread.SendCommand", (object[] args) => {
            if (args.Length == 2) {
                ...
                try {
                    ...
                    return new SendCommand(threadId, new List<Lib.ICommand>() {command});
                }
        ...
        IoC.Resolve<ICommand>("IoC.Register", "Thread.HardStopTheThread", (object[] args) => {
            if (args.Length == 1) {
                ...
                return new ThreadHardStopCommand(threadId, action);
            }
            else {
                throw new Exception();
            }
        ...
        IoC.Resolve<ICommand>("IoC.Register", "Thread.SoftStopTheThread", (object[] args) => {
            if (args.Length == 1) {
                ...
                var threadId = (string) args[0];
                var action = (Action) args[1];
                return new ThreadSoftStopCommand(threadId, action);
            }
            else {
                throw new Exception();
            }
        }).Execute();
        IoC.Resolve<ICommand>("IoC.Register", "Thread.Sender.MessageToCommand", (object[] args) => {
    ...
    public void Test_Thread_ExceptionHandler() {
        ...
        Assert.Equal(testString, "Command Exception");
    ...
    public void Test_ThreadStopCommandException() {
        ...
        var listcommands = new List<Lib.ICommand>();
        listcommands.Add(new ThreadStopCommand(thread1));
        listcommands.Add(new ThreadStopCommand(thread2));
